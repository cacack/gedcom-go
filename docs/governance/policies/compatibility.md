...
- **Some standard tags are recognized but not decoded into typed fields.** At the `INDI` level, `ALIA`, `AFN`, `RFN`, `RIN`, `ANCI`, `DESI`, `SUBM`, `RESN`, and `INIL` are preserved in `Individual.Tags` only — no typed accessor yet. `INIL` stays that way by decision ([#386](https://github.com/cacack/gedcom-go/issues/386)): a typed initiatory ordinance needs a sixth `gedcom.LDSOrdinanceType` constant that no consumer reads. At the `FAM` level, `RESN`, `SUBM`, and `ASSO` are the same case. `FACT` no longer is: `gedcom.Family` gained an `Attributes` field, so `FAM`-level `FACT` and `NCHI` decode into `Family.Attributes` and `CENS`/`RESI` into `Family.Events`, each with its `EVENT_DETAIL` ([#448](https://github.com/cacack/gedcom-go/issues/448)). None of these produce `UNKNOWN_TAG`, which is reserved for genuinely nonstandard tags ([#375](https://github.com/cacack/gedcom-go/issues/375)). The recognition tables for subordinate structures classify a small, enumerated set of standard tags as `UNKNOWN_TAG`: `FAMC` under `ADOP`/`BIRT`/`CHR`, `CREA`/`REFN`/`UID` on the `REPO`/`SUBM`/`SNOTE` records, and `NOTE` under an inline `OBJE` ([#470](https://github.com/cacack/gedcom-go/issues/470)). `docs/reference/gedcom-7-coverage.md` is the generated inventory — the `raw (flagged)` rows are exactly this set.
...
- **The nesting ceiling is not configurable.** It is fixed at `parser.MaxNestingDepth-1` (99) by the grammar's two-digit level field. `decoder.DecodeOptions.MaxNestingDepth` was an inert option that never lowered or raised it; v3 removed it ([#383](https://github.com/cacack/gedcom-go/issues/383)).
- **`INVALID_XREF` covers four different outcomes.** An XRef containing a space (`0 @NoTe ref@ NOTE x`), and a level-0 XRef with no closing `@` followed by a tag (`0 @I1 INDI`), are recovered and the record *kept* with a usable `Record.XRef`. A well-formed XRef with no tag (`0 @I1@`) is *dropped*, line and subordinates alike. `0 @I1` — unterminated **and** with no tag — is neither: the line keeps its pre-existing parse, so the record is kept but with `Type == "@I1"`, `XRef == ""` and `Entity == nil`, absent from `Document.XRefMap`, and `GetIndividual("@I1")` returns nil; `0 @I1 HEAD` and `0 @I1 TRLR` stay on that same path by decision ([#385](https://github.com/cacack/gedcom-go/issues/385)). Finally, a *well-formed* XRef on a structural line (`0 @X1@ HEAD`, `0 @X1@ TRLR`) becomes an ordinary record with `Type == "HEAD"`/`"TRLR"`, its identifier in `Document.XRefMap`, its subordinate lines intact, and `Entity == nil`: the GEDCOM grammar gives neither tag an identifier, so the line is not the document's header or trailer and the real header's typed fields are left alone ([#396](https://github.com/cacack/gedcom-go/issues/396)). All of these surface as `CodeInvalidXRef` at `SeverityError`, so severity does not distinguish them — check `Record.Type` and `Document.XRefMap` if you need to know which happened.
- **A `HEAD`- or `TRLR`-typed record is inert downstream.** It carries no `Entity`, so `Document.Individuals()`, `Notes()`, and the other typed accessors skip it; `converter` and `merge` treat it as any other unrecognized record type (`merge` renumbers or prefixes its XRef on collision); `validator` reports nothing for it; and `encoder` writes it back as `0 @X1@ HEAD` after the real header, so decode → encode → decode is stable. `version.DetectVersion` is the exception: it reads raw parser lines and still treats every level-0 `HEAD` as a header block, so a `GEDC`/`VERS` beneath a misidentified one can set `Document.Header.Version` when the real header declares no version of its own.
...
- **An XRef on a level >= 1 line lands on `gedcom.Tag.XRef`, not on a record.** Well-formed GEDCOM never puts an identifier there, so this only happens after recovery (`1 @I 1@ NOTE x`) or on input that was already odd (`1 @N1@ NOTE x`). The identifier is kept on the subordinate raw tag and re-encoded verbatim, so the document round-trips byte-exactly, but it is not a definition site: it never enters `Document.XRefMap` and `GetRecord` will not find it ([#395](https://github.com/cacack/gedcom-go/issues/395)). The re-encoded line stays as malformed as the input was — nothing is invented to make it parse. A subordinate XRef that *is* pointer-shaped and names a real record is remapped by `merge.RemapXRefs` along with every other reference to it. Two callers see the identifier that was previously invisible: `merge.RemapXRefs` rewrites it, and `gedcom.Document.Subset` now reports `unknown xref "@N9@" reachable from included records` for `1 @N9@ NOTE x` where no `@N9@` record exists — a call that used to succeed. That dangling reference was always in the file; dropping the identifier is what hid it, so surfacing it is the fix working, not a new failure.
